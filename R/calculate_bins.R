...
calculate_bins <- function(vcf, coord_map, bin_size, start_pos, end_pos){
  ...
  ## Get genotype data, make sure IDs are unique
  ...
  fix <- getFIX(vcf)
  if(!all(colnames(fix) == c('CHROM','POS','ID','REF','ALT','QUAL','FILTER'))){
    stop("Check VCF formatting - non-standard column name in first 7 of file")
  }
  ...
  if(any(grepl('/', gts))){
    stop("VCF must be haploid (for now)")
  }

  ## Filter to region
  in_region <- positions >= start_pos & positions <= end_pos ### T/F vector to filter w/
  ...
  ## Create bins in expanded coordinates
  ...
  for(b in 1:n_bins){
    ...
    if(any(in_bin)){ ### if there are any reference positions in the bin (i.e., bin doesn't just span an insertion)
  ...
  ## Initialize NA (missing) matrix for QC
  ## NA spaces should correspond to, e.g., an insertion/presence in other samples where
  ### the focal is spanned by a deletion
  missing_matrix <- matrix(0, nrow = length(samples), ncol = n_bins)
  rownames(missing_matrix) <- samples
  ...
  for(s in seq_along(samples)){
    ...
    for(v in seq_along(positions_region)){
      ...
      if(is.na(genotype) || genotype == "." || genotype == "./."){
        ...
        expanded_pos <- coord_map$expanded_pos[map_idx]
        missing_positions[expanded_pos] <- TRUE
        next
      }

      ## Parse genotype
      # alleles <- as.numeric(strsplit(gsub("[|/]", " ", genotype), " ")[[1]]) ## start point for diploid parsing
      allele <- as.numeric(genotype)

      ref_pos <- positions_region[v]
      ref_allele <- refs_region[v]
      alt_allele <- alts_region[v]
      ...
      for(allele_idx in allele){
        ...
        if(allele_idx == 0){
          ...
          allele_seq <- alt_allele[allele_idx]
    ...
    ### overlay deletions (even in VCF exported from MC graph w/ vg deconstruct, '0' alleles can show up
    ...
    ## Calculate +/- NA for each bin
    for(b in 1:n_bins){
      bin_positions <- bin_starts[b]:bin_ends[b]
      bin_positions <- bin_positions[bin_positions <= max_expanded]

      if(length(bin_positions) > 0){
        if(any(missing_positions[bin_positions])){
          missing_matrix[s, b] <- 1
        } else{
          missing_matrix[s, b] <- 0
        }
      }
    }
  }
